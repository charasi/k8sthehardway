pipeline {
    ...
    stages {
        stage('Run k8s-control-rbac.yml') {
            steps {
                script {
                    // Run the second playbook
                    def result = sh(script: "ansible-playbook -i hosts.hosts --private-key=$ANSIBLE_PRIVATE_KEY k8s-control-rbac.yml", returnStatus: true)
